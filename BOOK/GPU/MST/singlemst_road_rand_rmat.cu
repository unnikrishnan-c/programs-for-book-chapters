#include "singlemst_road_rand_rmat.h"
...
__global__ void resetlock(GGraph graph, /*SET*/ GSet set, int FALCX)
{
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		for (int kk = 0; kk < graph.npoints; kk += graphpointkernelblocks *TPB0)
		{
			resetlock <<<graphpointkernelblocks, TPB0>>> (graph, set, kk);
		}
	...
	mst = hreduxsum0;
	printf("\n correc if cost if<2^31 --->  mst cost=%lu\n", mst);

	mst = 0;

	#
	pragma omp parallel
	for num_threads(FALC_THREADS)
	for (int i = 0; i < hgraph.nedges; i++)
	{
		int falcft6 = 2 * i;

		if (((struct struct_hgraph *)(hgraph.extra))->mark[falcft6 / 2] == true)
			mst = /*HERE*/ mst + hgraph.edges[falcft6 + 1].ipe;
	}

	printf("\ncorrect for int and long int---> mst cost=%lu\n", mst);
