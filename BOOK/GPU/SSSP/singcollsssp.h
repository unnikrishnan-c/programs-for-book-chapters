...
void alloccoll1size(GColl & coll1)
{
	cudaMalloc((void **) &(coll1.size), sizeof(int));
	int temp = 0;
	cudaMemcpy(&(coll1.size[0]), &temp, sizeof(int), cudaMemcpyHostToDevice);
}

void alloccoll2size(GColl & coll2)
{
	cudaMalloc((void **) &(coll2.size), sizeof(int));
	int temp = 0;
	cudaMemcpy(&(coll2.size[0]), &temp, sizeof(int), cudaMemcpyHostToDevice);
}

void alloccoll3size(GColl & coll3)
{
	cudaMalloc((void **) &(coll3.size), sizeof(int));
	int temp = 0;
