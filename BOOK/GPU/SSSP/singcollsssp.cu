...
__global__ void relaxgraph(GGraph graph, GColl coll1, GColl coll2, int val, int FALCX)
{
	...
	if (id < graph.npoints)
	{
		...
		for (int falcft2 = 0; falcft2 < falcft0; falcft2++)
		{
			...
			if (((struct struct_hgraph *)(graph.extra))->dist[ut1] > newdist + ((struct struct_hgraph *)(graph.extra))->dist[id])
			{
				GMIN(&(((struct struct_hgraph *)(graph.extra))->dist[ut1]), newdist + ((struct struct_hgraph *)(graph.extra))->dist[id], ch);
...
void SSSP(char *name)
{
	...
	int falcvt2;
	cudaMemcpy(&falcvt2, &(coll1.size[0]), sizeof(int), cudaMemcpyDeviceToHost);
	coll1temp[falcvt2++] = 0;
	cudaMemcpy(&(coll1.size[0]), &falcvt2, sizeof(int), cudaMemcpyHostToDevice);
	...
	if (cudaMemcpy(&(temp1.dist[0]), &(falcvt3), sizeof(int), cudaMemcpyHostToDevice) != cudaSuccess) printf("memcpyerror 1");
