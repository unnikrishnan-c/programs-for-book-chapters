...
{
	public: int maxnpoints, pnpoints, maxnedges, pnedges;
	int init();
...
int HGraph::readMorph(char *file)
{
	...
	for (int i = 0; i < (npoints); i++)
	{
		...
		for (unsigned jj = 0; jj < total; ++jj)
		{
			...
			/*if(edgeindex > nedges*3){ printf("%d %d %d\n",nedges,edgeindex,i);
			exit(0);
			}*/
			unsigned dst = le32toh(outs[edgeindex - 1]);
...
int HGraph::readEdges(char *filename)
{
	...
	for (int i = 0; i < nedges; i++)
	{
		...
		if (src != currsrc)
		{
			for (int kk = currsrc + 1; kk < src; kk++)
			{
				//if some points/nodes does not have any edges
				index[kk] = ttindex;
			}
